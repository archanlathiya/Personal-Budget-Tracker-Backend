    ...
    if (categoryId as string) {
      const category = await db.select().from(categories).where(
        and(
          ...
          eq(categories.type, 'expense') // Budgets are typically for expenses
        )
      );
...
export const getBudgetById = async (c: Context) => {
  try {
    const user = c.get('user');
...
// Update a budget
export const updateBudget = async (c: Context) => {
  try {
    const user = c.get('user');
    const id = parseInt(c.req.param('id'));
    const { amount, month, year, categoryId } = await c.req.json();
    ...
    if ((month && month !== existingBudget[0].month) || 
      ...
      const duplicateBudget = await db.select().from(budgets).where(
        and(
          eq(budgets.userId, user.userId),
...
export const getBudgetVsActual = async (c: Context) => {
  try {
    const user = c.get('user');
